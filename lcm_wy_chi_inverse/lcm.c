...
int main(int argc, char *argv[]){
  ...
  if (argc != 7){
	  printf("LCM_WY_CHI: output_basefilename n_perm target_fwer input_class_labels_file input_transactions_file seed\n");
	  exit(1);
  }
  ...
  strcpy(tmp_filename,argv[1]); strcat(tmp_filename,"_minpvals.txt");
  ...
  trans_filename = argv[5];
  seed_idx = atoi(argv[6]);
  ...
  tic = measureTime();
  permutation_init(n_perm,class_labels_file,seed_idx);
  toc = measureTime();
