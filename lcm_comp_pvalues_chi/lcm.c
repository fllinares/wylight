...
int main(int argc, char *argv[]){
  ...
  if (argc != 6){
	  printf("ENUMERATE_SIGNIFICANT_ITEMSETS_CHI: output_basefilename corrected_significance_threshold minimum_testable_support input_class_labels_file input_transactions_file\n");
	  exit(1);
  }
  ...
  // Create a file to report results summary
  strcpy(tmp_filename,argv[1]); strcat(tmp_filename,"_summary.txt");
  if(!(results_file = fopen(tmp_filename,"w"))){
    	fprintf(stderr, "Error in function main when opening file %s\n",tmp_filename);
    	exit(1);
  }
  // Create output files for significant itemsets and correspoding pvalues
  strcpy(tmp_filename,argv[1]); strcat(tmp_filename,"_sig_itemsets.txt");
  if(!(significant_itemsets_output_file = fopen(tmp_filename,"w"))){
  	  fprintf(stderr, "Error in function enum_sig_itemsets_init when opening file %s\n",tmp_filename);
	  exit(1);
  }
  strcpy(tmp_filename,argv[1]); strcat(tmp_filename,"_sig_pvalues.txt");
  if(!(pvalues_output_file = fopen(tmp_filename,"w"))){
	  fprintf(stderr, "Error in function enum_sig_itemsets_init when opening file %s\n",tmp_filename);
	  exit(1);
  }
  // If the file was successfully create, write the file header
  fprintf(pvalues_output_file,"a,x,P-value\n");
  ...
  sig_th = atof(argv[2]);
  LCM_th = atoi(argv[3]); // Initialise the support of LCM to the input value
  class_labels_file = argv[4];

  // Remove arguments introduced by new functionality to avoid interference with the rest of LCM's code
  // Only remaining arguments at this point should be transactions_file = argv[1]
  argv[1] = argv[5]; argc = 2;
  ...
  // Initialize the code
  tic = measureTime();
  enum_sig_itemsets_init(sig_th,class_labels_file);
  toc = measureTime();
