...
void LCMclosed_BM_iter(int item, int m, int pmask){
  ...
  if(LCM_frq <= LCM_th_upper) {dataset_frq += LCM_frq; n_itemsets++;}
...
int main(int argc, char *argv[]){
  ...
  if (argc < 4){
      printf("LCM_WY: low_sup high_sup trans_list_filename [output-filename]\n");
      exit(1);
  }
  ...
  argc -= 2;
  for(i=1; i<argc; i++) argv[i] = argv[i+2];
