...
long long dataset_frq, n_itemsets;
...
void transaction_keeping_end(){
	...
	// Close file
	printf("Total number of frequent itemsets with support %d: %lld\n",LCM_th,n_itemsets);
	printf("Total dataset frequency with support %d: %lld\n",LCM_th,dataset_frq);
	printf("Memory usage lower bound: %f (MB).\n",(n_itemsets*sizeof(int *) + dataset_frq*sizeof(int))/((double)1048576));
}
